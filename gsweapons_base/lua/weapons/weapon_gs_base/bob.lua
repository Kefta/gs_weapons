...
AddBobbingMethod( "hl2", function( pWeapon, _, vOrigin, ang )
	...
	if ( flBobCycle and flBobUp and flBobCycle > 0 and flBobUp > 0 and flBobUp < 1 and FrameTime() ~= 0 ) then
		...
		//		 MaxSpeed() is not sufficient for dealing with sprinting - jdw
		local flBobSpeed = pWeapon.BobSpeed * pPlayer:GetWalkSpeed()
		local flSpeed = math.Clamp( pPlayer:_GetLocalVelocity(), -flBobSpeed, flBobCycle )
		
		local flBobOffset = math.RemapVal( flSpeed, 0, flBobSpeed, 0, 1 )
		local flCurTime = CurTime()
		flBobTime = (flCurTime - flLastBobTime) * flBobOffset
		flLastBobTime = flCurTime
		...
		if ( flCycle < flBobUp ) then
			...
			flCycle = pi + pi * (flCycle - flBobUp) / (1 - flBobUp)
		end
		...
		// Calculate the lateral bob
		flCycle = (flBobTime - math.floor( flTimeByCycle * 2 ) * flBobCycle * 2) / (flBobCycle * 2)
