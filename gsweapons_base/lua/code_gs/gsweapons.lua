...
function PLAYER:SharedRandomFloat( sName, flMin, flMax, iAdditionalSeed )
	random.SetSeed( util.SeedFileLineHash( hash.PseudoRandom( self:GetCurrentCommand():CommandNumber() ) % 0x80000000, sName, iAdditionalSeed ))
...
function PLAYER:LuaFireBullets( bullets )
	...
	local iSeed = hash.PseudoRandom( self:GetCurrentCommand():CommandNumber() ) % 0x100 - 1
...
function PLAYER:FireCSBullets( bullets )
	...
	// Wrap it for network traffic so it's the same between client and server
	local iSeed = hash.PseudoRandom( self:GetCurrentCommand():CommandNumber() ) % 0x100
