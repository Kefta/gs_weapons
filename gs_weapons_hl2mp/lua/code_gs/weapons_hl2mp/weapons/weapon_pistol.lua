...
function SWEP:Punch()
	...
	pPlayer:ViewPunch(Angle(code_gs.random:SharedRandomFloat(pPlayer, "pistolpax", tPunch.XMin, tPunch.XMax),
		code_gs.random:SharedRandomFloat(pPlayer, "pistolpay", tPunch.YMin, tPunch.YMax), 0))
end
...
function SWEP:GetSpecialKey(sKey, bSecondary, bNoConVar)
	if (sKey == "Spread") then
		...
		return LerpVector(math.Remap(self.dt.AccuracyPenalty, 0, self.MaxAccuracyPenalty, 0, 1), BaseClass.GetSpecialKey(self, sKey, false, bNoConVar), BaseClass.GetSpecialKey(self, sKey, true, bNoConVar))
	end
