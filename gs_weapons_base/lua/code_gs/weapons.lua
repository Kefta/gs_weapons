...
do
	...
	function weapons.Get( name )
		...
		local retval = table.DeepCopy( Stored )
...
do
	...
	function scripted_ents.Get( name )
		...
		-- Create/copy a new table
		local retval = table.DeepCopy( Stored.t )
...
function code_gs.weapons.ActualClassName(sName, sAddon)
	if (not sAddon) then
		return sName:lower()
	end
	...
	if (tNameConVars[sAddon]) then
		return tNameConVars[sAddon]:GetBool() and sName:lower() or "gs_" .. sAddon .. '_' .. sName:lower()
	end
	
	ErrorNoHalt("ConVar for \"" .. sAddon .. "\" not found! Sending back default name..")
	
	return sName:lower()
end

local function SetupVariables(tbl, sClass, sBase, sCategory, sAddon)
	if (CLIENT and not tbl.Category) then
		tbl.Category = sCategory
	end
	
	if (not tbl.Base) then
		tbl.Base = sBase or nil
	elseif (tbl.Base:sub(1, 3):lower() == "gs_") then
		tbl.Base = tbl.Base:lower()
	else
		tbl.Base = tbl.Base and code_gs.weapons.ActualClassName(tbl.Base, sAddon)
	end
	
	-- FIXME: Add language change hooks
	if (tbl.PrintName) then
		local sName = tbl.PrintName:lower()
		
		if (sName:sub(1, 3) == "gs_") then
			tbl.PrintName = code_gs.GetPhrase(sAddon and ("weapons_" .. sAddon .. "_" .. sName:sub(4)) or ("weapons_" .. sName:sub(4)))
		end
	else
		tbl.PrintName = code_gs.GetPhrase(sAddon and ("weapons_" .. sAddon .. "_" .. sClass) or ("weapons_" .. sClass))
	end
end
	...
	for i = 1, #tFiles do
		...
		if (sFile:sub(-4) == ".lua") then
			...
			if (func) then
				...
				local tEnv = setmetatable(table.DeepCopy(tEnv), GLOBALENV)
				tEnv._ENV = tEnv
				debug.setfenv(func, tEnv)
...
function code_gs.weapons.RegisterWeaponsFromFolder(sPath, sCategory, sAddon)
	Register(tSWEPEnv, sPath, function(tEnv, sClass)
		local SWEP = tEnv.SWEP
		sClass = SWEP.ClassName or sClass
		SWEP.ClassName = code_gs.weapons.ActualClassName(sClass, sAddon)
		SetupVariables(SWEP, sClass, "gs_baseweapon", sCategory, sAddon)
...
function code_gs.weapons.RegisterEntitiesFromFolder(sPath, sCategory, sAddon)
	Register(tENTEnv, sPath, function(tEnv, sClass)
		local ENT = tEnv.ENT
		sClass = ENT.ClassName or sClass
		ENT.ClassName = code_gs.weapons.ActualClassName(sClass, sAddon)
		SetupVariables(ENT, sClass, "gs_baseentity", sCategory, sAddon)
...
function code_gs.weapons.GetEntityPrintName(pEntity)
	return pEntity.PrintName or pEntity.GetPrintName and pEntity:GetPrintName() or code_gs.GetPhrase(pEntity:GetClass())
end

function code_gs.weapons.RegisterOverrideConVar(sName)
	sName = sName:lower()
	tNameConVars[sName] = CreateConVar("gs_weapons_" .. sName .. "_override", "0", bit.bor(FCVAR_ARCHIVE, FCVAR_REPLICATED), "Remove gs_ from weapon names. THIS REQUIRES A CHANGELEVEL/SERVER RESTART AFTER BEING CHANGED.")
end

CreateConVar("gs_weapons_usecmodels", "0", bit.bor(FCVAR_ARCHIVE, FCVAR_REPLICATED), "Use c-models for weapons, where applicable")
...
do
	...
	function code_gs.weapons.GetHoldType(sName)
		...
		code_gs.DevMsg(2, string.format("[GSWeapons] (GetHoldType) HoldType %q not found. Sending back table for \"normal\".", sName))
