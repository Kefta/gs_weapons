...
function SWEP:SmackDamage( tr, vForward, bSecondary )
	...
	local info = DamageInfo()
		...
		if ( self.m_iCombo == 2 ) then
			...
			info:SetDamage( gsrand:RandomInt( flDamage, flDamage * 2 ))
			--info:SetDamageForce()
			-- Figure out how the exact force numbers are calculated
		else
			info:SetDamage( gsrand:RandomInt( self:GetSpecialKey( "Damage", bSecondary ), self:GetSpecialKey( "MaxDamage", bSecondary )))
		end
